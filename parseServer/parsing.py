...
def get_total_avg_dp(request, db_stub_list, pk_ctx, key_stub):
    # keyServer generates noises foe the qid-th request
    generate_noise_request = request_keyServer_pb2.requestGenerateNoise(cid=request.cid, qid=request.qid,
                                                                        type="float")
    key_stub.GenerateNoise(generate_noise_request)
...
def get_total_avg(request, db_stub_list, pk_ctx, key_stub):
    ...
    # get the average(sum/count) by calling the division interface provided by keyServer
    total_enc_sum_msg = total_enc_sum.serialize()
    total_enc_count_msg = total_enc_count.serialize()
    div_request = request_keyServer_pb2.divVectors(dividendMsg=total_enc_sum_msg, divisorMsg=total_enc_count_msg)
    div_response = key_stub.DivEncVector(div_request)
    total_enc_avg_msg = div_response.vectorMsg
...
def get_mid_total_var(request, db_stub_list, pk_ctx, key_stub):
    ...
    return total_enc_avg, total_enc_count, total_enc_mid_result


def get_total_var(request, db_stub_list, pk_ctx, key_stub):
    total_enc_avg, total_enc_count, total_enc_mid_result = get_mid_total_var(request, db_stub_list, pk_ctx, key_stub)
    # get the division result:total_mid_result/total_count
    total_enc_mid_result_msg = total_enc_mid_result.serialize()
    total_enc_count_msg = total_enc_count.serialize()
    div_request = request_keyServer_pb2.divVectors(dividendMsg=total_enc_mid_result_msg, divisorMsg=total_enc_count_msg)
    div_response = key_stub.DivEncVector(div_request)
    div_enc_mid_msg = div_response.vectorMsg
...
def get_total_var_samp(request, db_stub_list, pk_ctx, key_stub):
    ...
    sub_mid_result = total_enc_mid_result - total_enc_avg * total_enc_avg * total_enc_count
    sub_mid_msg = sub_mid_result.serialize()
    divisor_vector = total_enc_count - 1
    divisor_msg = divisor_vector.serialize()

    div_request = request_keyServer_pb2.divVectors(dividendMsg=sub_mid_msg, divisorMsg=divisor_msg)
    div_response = key_stub.DivEncVector(div_request)
    total_enc_var_samp_msg = div_response.vectorMsg
    total_enc_var_samp = ts.ckks_vector_from(pk_ctx, total_enc_var_samp_msg)
...
def get_total_std(request, db_stub_list, pk_ctx, key_stub):
    ...
    sqrt_request = request_keyServer_pb2.vectorResult(vectorMsg=total_enc_var_msg)
    sqrt_response = key_stub.SqrtEncVector(sqrt_request)
    total_enc_std_msg = sqrt_response.vectorMsg
...
def get_total_std_samp(request, db_stub_list, pk_ctx, key_stub):
    ...
    sqrt_request = request_keyServer_pb2.vectorResult(vectorMsg=total_enc_var_samp_msg)
    sqrt_response = key_stub.SqrtEncVector(sqrt_request)
    total_enc_std_samp_msg = sqrt_response.vectorMsg
