...
def request_parsing(request, pk_ctx, address_dict, options):
    ...
    if db_name == "TOTAL":
        ...
        enc_vector = process_local_requst(request, pk_ctx, db_stub)

        return enc_vector


# process the request which query data from local database
def process_local_requst(request, pk_ctx, db_stub):
...
def get_total_sum(request, stub_list, pk_ctx):
    ...
    for stub in stub_list:
        enc_vector = process_local_requst(request, pk_ctx, stub)
